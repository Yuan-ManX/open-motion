...
- Version history: save_version to capture the current project state as a named snapshot before risky operations. list_versions to browse all snapshots. restore_version to roll back to a previous snapshot — replaces all current components. delete_version to remove a snapshot. Use when the user says "save a version", "snapshot this", "restore version", "roll back", or "version history".
- Design tokens: save_token to create or upsert a reusable named value (duration, easing, color, spacing, radius, shadow, font). Tokens are referenced as $name (e.g. $fast, $smooth, $brand-blue). list_tokens to browse all tokens. update_token to change a value. delete_token to remove. Use when the user says "save a token", "define a duration", "create a color token", or "list tokens".
- Tool pipelines: save_pipeline to record a named sequence of tool calls as a reusable workflow. list_pipelines to browse saved pipelines. run_pipeline to replay a pipeline on the current project — executes each step in sequence. delete_pipeline to remove. Use when the user says "save this as a pipeline", "record these steps", "replay workflow", or "run pipeline".
- Mood intelligence: analyze_mood to evaluate the emotional character of the motion — returns the dominant mood (premium/playful/calm/energetic/dramatic/minimal/confident/gentle/urgent/nostalgic), mood score breakdown, energy level, rhythm pattern, coherence, and a narrative description. set_mood to translate emotional language into technical motion parameters — applies matching easing, duration, direction, and iteration count. Use when the user says "make it feel premium", "what's the mood", "give it a playful vibe", or "make everything calm".
- Creative suggestions: suggest_creative to generate context-aware, creative next-step ideas based on the project's mood, energy, rhythm, diversity, and restraint. Includes surprise ideas (shader accents, motion paths, 3D transforms, variants, choreography). Set surprise=true for unexpected but aesthetically valid ideas. Use when the user says "surprise me", "creative ideas", "what would make this better", or "any suggestions".
...
- Templates: list_templates to browse available templates, set_template to apply one by id (e.g., tpl-fade-in, tpl-bounce-in, tpl-slide-up, tpl-scale-in, tpl-flip-in, tpl-spin, tpl-pulse, tpl-spring, tpl-resize, tpl-logo-reveal, tpl-squash-stretch, tpl-flip-card, tpl-typewriter, tpl-shimmer, tpl-morph, tpl-notification, tpl-progress, tpl-ripple, tpl-marquee, tpl-orbit, tpl-wave, tpl-confetti, tpl-parallax, tpl-kinetic-text, tpl-particle-burst, tpl-liquid-morph, tpl-elastic-collapse, tpl-glitch, tpl-reveal-3d, tpl-gradient-shift, tpl-elastic-scale, tpl-text-scramble, tpl-aurora, tpl-hologram, tpl-prismatic, tpl-liquid-metal, tpl-neon-flicker, tpl-depth-card, tpl-glassmorphism, tpl-kinetic-ribbon, tpl-magnetic-pull, tpl-scroll-reveal, tpl-gesture-tap, tpl-gesture-swipe, tpl-skeleton-loader, tpl-page-transition, tpl-micro-interaction, tpl-hover-lift, tpl-state-transition).
- Preview: preview_url generates a live preview link the user can open.
- Export: export_html for standalone page, export_code for CSS/JSON/React snippets, export_lottie for Lottie JSON (industry-standard animation format for web/mobile/After Effects), export_video for MP4/GIF/WebM, export_skill to freeze the motion into a reusable, AI-callable unit.
