...
import type {
  CursorChoreography,
  CursorPattern,
  CursorChoreographyMatchResult,
} from "../../api/endpoints.js";

const PATTERNS: CursorPattern[] = [
  "spotlight",
  "magnet",
  "trail",
  "repel",
...
const PATTERN_GLYPH: Record<CursorPattern, string> = {
  spotlight: "◉",
  magnet: "✦",
  trail: "〜",
  repel: "◌",
...
/**
 * Cursor Choreography panel — surfaces the cursor-reactive scene presets.
 * Each preset defines a cursor-driven pattern (spotlight, magnet, trail,
 * repel, tug, reveal, wake) with radius, intensity, easing, and timing.
 * Includes a free-text matcher that resolves a natural-language hint to the
 * closest preset so the designer can describe an interaction feel and get a
 * matching cursor choreography.
 */
export function CursorChoreographyPanel() {
  const [presets, setPresets] = useState<CursorChoreography[]>([]);
  const [loading, setLoading] = useState(true);
  const [activePattern, setActivePattern] = useState<string>("all");
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [matchQuery, setMatchQuery] = useState<string>("");
  const [matchResult, setMatchResult] = useState<CursorChoreographyMatchResult | null>(null);
  ...
  const filtered = useMemo(() => {
    if (activePattern === "all") return presets;
    return presets.filter((p) => p.pattern === activePattern);
  }, [presets, activePattern]);
  ...
  const handleMatch = () => {
    ...
    setMatching(true);
    api
      .matchCursorChoreography(q)
      .then(setMatchResult)
      .catch(() => setMatchResult({ query: q, match: null }))
      .finally(() => setMatching(false));
  };

  const handleCopyId = async (preset: CursorChoreography) => {
    try {
  ...
  if (loading) {
    return <div className="p-4 text-xs text-gray-500">Loading cursor presets…</div>;
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-2 py-1.5 border-b border-edge bg-panel2 flex-shrink-0">
        <div className="text-[9px] text-gray-500 flex items-center gap-1">
          ...
          <span>Cursor choreography ({presets.length})</span>
        </div>
      </div>
      ...
      <div className="px-2 pt-2 pb-1.5 border-b border-edge flex-shrink-0">
        <div className="relative">
          <input
            ...
            placeholder="Describe a cursor feel — “playful grid”, “magnetic cta”…"
          ...
          <button
            ...
            aria-label="Match cursor choreography"
          >
            {matching ? "…" : "match"}
          </button>
        </div>
      ...
      <div className="flex gap-1 px-2 pt-1.5 pb-1 flex-wrap flex-shrink-0">
        ...
        {PATTERNS.map((p) => {
          const count = presets.filter((x) => x.pattern === p).length;
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-1.5">
        {filtered.map((preset) => {
          ...
          const intensityPct = Math.round(preset.intensity * 100);
          return (
            <div
              key={preset.id}
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  <span className="text-[8px] text-gray-600 font-mono w-10 flex-shrink-0">
                    intensity
                  </span>
                  ...
                  <span className="text-[8px] text-gray-500 font-mono w-7 text-right">
                    {intensityPct}%
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-1 text-[8px] text-gray-600 font-mono">
                  <span>◎ r{preset.radius}</span>
                  <span>⌛ {preset.durationMs}ms</span>
                  <span>◇ {preset.idealElementCount} el</span>
                  {preset.trailsCursor && <span>↪ trails</span>}
                </div>
              </button>
              ...
              {isExpanded && (
                <div className="px-2 pb-2 pt-0.5 border-t border-edge/50 space-y-1.5">
                  ...
                  <div>
                    ...
                    <EasingRow easing={preset.easing} />
                  </div>

                  {/* Behavior flags */}
                  <div className="grid grid-cols-2 gap-1 text-[9px]">
                    <FlagChip label="trails cursor" on={preset.trailsCursor} />
                    <FlagChip
                      label="ideal element count"
                      on={preset.idealElementCount >= 8}
                      value={String(preset.idealElementCount)}
                    />
                  </div>
...
function EasingRow({
  ...
  return (
    <div className="flex items-center gap-1.5">
      {label && <span className="text-gray-500 w-14 flex-shrink-0">{label}</span>}
...
function FlagChip({
  ...
  return (
    <div
      className={`flex items-center justify-between px-1.5 py-0.5 rounded border text-[9px] ${
        on
          ? "border-accent/40 text-gray-300"
          : "border-edge text-gray-600"
      }`}
    >
      <span>{label}</span>
      <span className="font-mono">{value ?? (on ? "on" : "off")}</span>
    </div>
  );
}
